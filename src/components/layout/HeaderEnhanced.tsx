    ...
    <>
      <header
        ...
        className={cn(
          ...
          isScrolled && 'bg-white/98 backdrop-blur-lg shadow-sm border-b border-jade-green/8',
        ...
        <div className="container mx-auto px-8">
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-10">
                ...
                >
                  ...
                  <span className="font-light">+507 123-45678</span>
                </a>
                <a
          ...
          <div className="flex justify-between items-center h-18 lg:h-20">
              ...
              {navigationData.map((item) => (
                ...
                >
                  {item.hasDropdown ? (
                    <button
                      className={cn(
                        'flex items-center space-x-1 px-5 py-5 text-xs font-light tracking-wide transition-all duration-300',
                        activeDropdown === item.id ? 'text-jade-green/90' : 'text-gray-700 hover:text-jade-green/80'
                      )}
                    ...
                    >
                      <span className="uppercase letter-spacing-1">{t(item.label)}</span>
                    ...
                    <Link
                      ...
                      className={cn(
                        'relative px-5 py-5 text-xs font-light tracking-wide transition-all duration-300',
                    ...
                    <div className={cn(
                      ...
                      {item.items?.map((subItem) => (
                        <div
                          ...
                          onMouseEnter={() => subItem.hasSubmenu && openSubmenu(subItem.id)}
                          onMouseLeave={() => subItem.hasSubmenu && closeSubmenu()}
                        >
                          {subItem.hasSubmenu ? (
                            <Link
                              href={getLocalizedUrl(subItem.href || '/', locale)}
                              className={cn(
                                'flex items-center justify-between w-full px-6 py-3 text-xs font-light tracking-wide transition-colors duration-300',
                                activeSubmenu === subItem.id ? 'text-jade-green/80 bg-gray-50/50' : 'text-gray-700 hover:text-jade-green/80 hover:bg-gray-50/30'
                              )}
                            ...
                            <Link
                              href={getLocalizedUrl(subItem.href || '/', locale)}
                              className={cn(
                                'block px-6 py-3 text-xs font-light tracking-wide transition-colors duration-300',
                          ...
                          {subItem.hasSubmenu && (
                            <div className={cn(
                              ...
                              {subItem.items?.map((subSubItem) => (
                                <Link
                                  key={subSubItem.id}
                                  href={getLocalizedUrl(subSubItem.href, locale)}
                                  className={cn(
                                    'block px-6 py-3 text-xs font-light tracking-wide transition-colors duration-300',
                                  ...
                                  onClick={() => handleNavigation(subSubItem.href)}
                                >
                                  {t(subSubItem.label)}
                                </Link>
                              ))}
            ...
            <div className="flex items-center space-x-3">
              ...
              <Button
                asChild
                className="hidden md:inline-flex bg-jade-green/90 hover:bg-jade-green text-white px-5 py-2 text-xs font-light tracking-wide transition-all duration-300 hover:shadow-md hover:-translate-y-px"
              >
